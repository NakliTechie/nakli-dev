/* Generated from LocalMind index.html #bonsaiEngineSrc. Run node scripts/extract-image-worker.mjs after changing the inline engine. */
...
/* ============================================================================
 * LocalMind <-> Bonsai image-engine worker protocol  (engine = lines 31-13984
 * of webml-community/bonsai-image-webgpu prettified bundle; public class Bi).
 * ==========================================================================*/
...
self.onmessage = async (ev) => {
  ...
  try {
    switch (m.type) {
      case 'probe': {
        let webgpu = !!(self.navigator && self.navigator.gpu), adapter = false, info = null;
        if (webgpu) {
          try {
            const a = await navigator.gpu.requestAdapter({ powerPreference: 'high-performance' });
            adapter = !!a;
            if (a && a.requestAdapterInfo) { try { info = await a.requestAdapterInfo(); } catch (e) {} }
          } catch (e) { adapter = false; }
        }
        reply({ type: 'probe:result', webgpu, adapter, info });
        break;
      }
      case 'load': {
        if (__pipe) { reply({ type: 'loaded', already: true }); break; }
        __pipe = await Bi.from_pretrained(m.modelId != null ? m.modelId : null, {
          onProgress: (p) => reply(Object.assign({ type: 'progress' }, p)),
        });
        reply({ type: 'loaded' });
        break;
      }
      case 'generate': {
        if (!__pipe) throw new Error('image model not loaded');
        if (__busy) throw new Error('a generation is already in progress');
        __busy = true;
        const steps = Number(m.steps != null ? m.steps : 4);
        const res = await __pipe.generate({
          prompt: m.prompt,
          width: Number(m.width != null ? m.width : 512),
          height: Number(m.height != null ? m.height : 512),
          numInferenceSteps: steps,
          seed: Number(m.seed != null ? m.seed : 0),
          callbackOnStepEnd: (_p, step) => reply({ type: 'step', step: step + 1, total: steps }),
          log: (s) => reply({ type: 'log', message: s }),
        });
        __busy = false;
        reply({ type: 'image', id: m.id != null ? m.id : null, bytes: res.bytes, width: res.width, height: res.height, seed: res.seed });
        break;
      }
      case 'destroy': {
        try { if (__pipe && __pipe.destroy) await __pipe.destroy(); } catch (e) {}
        __pipe = null; __busy = false;
        reply({ type: 'destroyed' });
        break;
      }
      default:
        reply({ type: 'error', message: 'unknown message type: ' + m.type });
    }
  } catch (err) {
    __busy = false;
    reply({ type: 'error', message: String((err && err.message) || err), stack: (err && err.stack) || null });
  }
};
