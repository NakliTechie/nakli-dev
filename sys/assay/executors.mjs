...
// each role is handed only a role-scoped VIEW of the ledger — the Implementer never
// receives findings or instrument cases (only directives), the Foreman receives finding
...
export const ROLE_INSTRUCTIONS = {
  assayer:
    'You are the ASSAYER. Author and run the standard of completion BEFORE the implementer builds. ' +
  ...
  implementer:
    'You are the IMPLEMENTER. Build the candidate. You receive directives at the subsystem/behaviour level — a directive tells you WHERE the candidate is weak, not the answer. Probe the oracle first. ' +
  ...
  foreman:
    'You are the FOREMAN. Hold the whole outcome. Decide which findings are real; reject noise with a reason. Turn the rest into a directive ordered by measured weight, at the level of missing features/subsystems/behaviour — never naming a case. ' +
...
const ACTIONS = {
  ...
  'build-candidate': {
    ...
    build: (a, ctx) => ({ type: 'assay.candidate', round: ctx.round, git_ref: String(a.git_ref || ('c' + ctx.round)), from_directive: ctx.round >= 1 ? String(a.from_directive || '') : null, implementer_tests: { count: Number(a.test_count) || 0, deleted: Number(a.deleted) || 0 }, ...(a.test_change_reason ? { test_change_reason: String(a.test_change_reason) } : {}) }),
  },
  measure: {
    role: 'assayer',
  ...
  'propose-ship': {
    ...
    build: (a, ctx) => ({ type: 'assay.ship', round: ctx.round, candidate_ref: String(a.candidate_ref || ''), foreman_rationale: String(a.rationale || '') }),
  },
};
