...
const ASSAYER = 'actor:agent:assayer:c1';
const IMPL = 'actor:agent:implementer:c1';
const FOREMAN = 'actor:agent:foreman:c1';
const NOW = 2000;
...
async function cleanLedger() {
  ...
  await L.append({ type: 'assay.instrument.v1', campaign: C, actor: ASSAYER, ts: 2, version: 1, ratchet_sha: 'sha256:aaa', areas: [], case_count: 0 });
  await L.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 3, round: 0, git_ref: 'c0', implementer_tests: { count: 5, skipped: 0, deleted: 0 } });
  await L.append({ type: 'assay.finding.v1', campaign: C, actor: ASSAYER, ts: 4, round: 0, instrument_version: 1, clusters: [{ id: 'cl1', area: 'view', weight: 3 }] });
  await L.append({ type: 'assay.directive.v1', campaign: C, actor: FOREMAN, ts: 5, round: 1, id: 'd1', from_findings: ['cl1'], items: [{ area: 'view', weight: 3, instruction: 'implement -H header' }] });
  await L.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 6, round: 1, git_ref: 'c1', from_directive: 'd1', implementer_tests: { count: 7, skipped: 0, deleted: 0 } });
  return L;
}
...
await test('three role manifests mint, each a set of scoped grants', async () => {
  ...
  const impl = await mint(rk, 'implementer', IMPL);
  eq(impl.grants.length, 6, 'implementer holds 6 grants');
  const assr = await mint(rk, 'assayer', ASSAYER);
  assert(assr.grants.length >= 5, 'assayer holds its grant set');
  assert(PERSON_ONLY.includes('ship.accept'), 'ship.accept is declared person-only');
});

await test('the WALL: implementer is denied the raw instrument scope; assayer is granted it', async () => {
  const rk = newRootKey();
  const impl = await mint(rk, 'implementer', IMPL);
  const assr = await mint(rk, 'assayer', ASSAYER);
  const target = `assay:${C}:instrument/case/42`;
  const dImpl = await authorize(impl, rk, ctx(IMPL, 'read', target));
  assert(!dImpl.ok, 'implementer read of a raw instrument case must be denied');
  const aAssr = await authorize(assr, rk, ctx(ASSAYER, 'read', target));
  assert(aAssr.ok, 'assayer read of the instrument must be allowed');
});

await test('foreman sees the summary leaf but NOT raw cases (scope disjointness)', async () => {
  const rk = newRootKey();
  const fore = await mint(rk, 'foreman', FOREMAN);
  const summary = await authorize(fore, rk, ctx(FOREMAN, 'read', `assay:${C}:instrument:summary`));
  assert(summary.ok, 'foreman may read the instrument summary');
  const rawCase = await authorize(fore, rk, ctx(FOREMAN, 'read', `assay:${C}:instrument/case/42`));
  assert(!rawCase.ok, 'foreman must NOT reach a raw instrument case');
});

await test('implementer owns its candidate (read+write) but cannot write the instrument', async () => {
  const rk = newRootKey();
  const impl = await mint(rk, 'implementer', IMPL);
  assert((await authorize(impl, rk, ctx(IMPL, 'write', `assay:${C}:candidate`))).ok, 'implementer writes candidate');
  assert(!(await authorize(impl, rk, ctx(IMPL, 'write', `assay:${C}:instrument`))).ok, 'implementer cannot write instrument');
});

await test('a wrong-principal grant does not authorize (principal caveat binds)', async () => {
  const rk = newRootKey();
  const assr = await mint(rk, 'assayer', ASSAYER);
  // Present the assayer's grant set but claim to be the implementer.
...
await test('guardedAccess: a denied instrument reach records an assay.wall.breach', async () => {
  ...
  eq(breaches[0].actor, IMPL, 'breach attributes the implementer');
...
await test('an ALLOWED access records no breach', async () => {
  ...
  const assr = await mint(rk, 'assayer', ASSAYER);
  const L = createAssayLedger();
  const res = await guardedAccess(assr, rk, ctx(ASSAYER, 'read', `assay:${C}:instrument`), L, { campaign: C });
  assert(res.ok, 'assayer instrument read allowed');
...
await test('validateBlock accepts known blocks, allows forward assay.* types, rejects the rest', async () => {
  ...
  assert(validateBlock({ type: 'assay.finding.v1', campaign: C, actor: ASSAYER, ts: 1, clusters: [] }), 'finding with empty clusters rejected');
});
...
await test('verify tests fails on a deleted test with no reason, passes with a reason', async () => {
  ...
  await bad.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 1, round: 0, git_ref: 'a', implementer_tests: { count: 5, deleted: 0 } });
  await bad.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 2, round: 1, from_directive: 'd', implementer_tests: { count: 4, deleted: 2 } });
  eq(verifyTests(bad, { campaign: C }).code, 1, 'deleted tests with no reason fails');

  const ok = createAssayLedger();
  await ok.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 1, round: 0, git_ref: 'a', implementer_tests: { count: 5, deleted: 0 } });
  await ok.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 2, round: 1, from_directive: 'd', implementer_tests: { count: 4, deleted: 2 }, test_change_reason: 'merged duplicate suites' });
...
await test('verify tests fails on a silent count decrease', async () => {
  const L = createAssayLedger();
  await L.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 1, round: 0, git_ref: 'a', implementer_tests: { count: 9, deleted: 0 } });
  await L.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 2, round: 1, from_directive: 'd', implementer_tests: { count: 6, deleted: 0 } });
...
await test('replay fails on a directive citing an unknown finding', async () => {
  const L = createAssayLedger();
  await L.append({ type: 'assay.finding.v1', campaign: C, actor: ASSAYER, ts: 1, round: 0, clusters: [{ id: 'cl1' }] });
  await L.append({ type: 'assay.directive.v1', campaign: C, actor: FOREMAN, ts: 2, round: 1, id: 'd1', from_findings: ['ghost'], items: [] });
...
await test('replay fails on a candidate citing an unknown directive', async () => {
  const L = createAssayLedger();
  await L.append({ type: 'assay.finding.v1', campaign: C, actor: ASSAYER, ts: 1, round: 0, clusters: [{ id: 'cl1' }] });
  await L.append({ type: 'assay.directive.v1', campaign: C, actor: FOREMAN, ts: 2, round: 1, id: 'd1', from_findings: ['cl1'], items: [] });
  await L.append({ type: 'assay.candidate', campaign: C, actor: IMPL, ts: 3, round: 1, from_directive: 'ghost', implementer_tests: { count: 1 } });
